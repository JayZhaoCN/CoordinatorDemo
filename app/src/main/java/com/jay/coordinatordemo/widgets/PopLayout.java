...
import com.jay.coordinatordemo.Utils;
...
public class PopLayout extends ViewGroup implements View.OnClickListener{
    ...
    //供外部调用
    // FIXME: 2017/1/5 需要实现一种没有动画，直接隐藏的效果，供Activity之间跳转使用，否则会有卡顿
    public void close() {
        if(mStyle == STYLE_POP) {
            initPopAnim();
        } else {
        ...
        if(mIcons.get(mIcons.size() - 1) instanceof CrossBottomView) {
            ((CrossBottomView) mIcons.get(mIcons.size() - 1)).initAnim(false);
        }
    }
